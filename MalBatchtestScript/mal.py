# -*- coding: utf-8 -*-
import sys,os,time,json,ConfigParser,urllib,urllib2,codecs,requests,ast
#import pandas as pd
...
def getClassificationAPIResponse(params, modelUrlName,req_no):
    ...
    scxmlUrl = params['scxml_url']
    web2nlKey = params['web2nlKey']
    ...
    q1 =str(queryString)
    ### Note: Change web2nl keys as per environment
    data = {
       'scxml': scxmlUrl,
       'loaddata': '{"q": "'+q1+'","models":"root_intent","client": "columbia","appname": "general","malname": "columbia_mal", "rtdp_key":"3YeYiRm7uu1J0ta3","web2nl_key": "'+web2nlKey+'","verbose":"false","maxintents":1}',
        'deferResponse': 'true'
    }
    responseString=''
    retry=True
    ...
    while(retry and (count<max_retries)):
        try:
            time.sleep(.1)
            count+=1
            response = requests.post(requestUrl, headers=headers, data=data,timeout=5)
...
def get_classified_output(params,cols,intent_model_url,root_intent,out_of_domain,line_no,is_hierarchy,custom_encoding):
    ...
    params['request_url'] = intent_model_url['requestUrl']
    params['scxml_url'] = intent_model_url['scxml']
    params['web2nlKey'] = intent_model_url['web2nlKey']
    ...
    if(intent_line.strip()):
        ...
        first_level_response1 = getClassificationAPIResponse( params, 'model_url',line_no)
        first_level_response= first_level_response1['events']
        
        if(verbose_out_normalization!=""):
            verbose_out= verbose_out_normalization
        verbose_out += '\t' +str(first_level_response)
        
        malresponse= json.loads(first_level_response[0]['data']['malResponse'])
        
        response_final = (ast.literal_eval(json.dumps(malresponse)))
                       
        LAST_INPUT='lastInput'
        INTENTS = 'intents'
        CLASSNAME = 'className'
        SCORE ='score'
        last_input_string=response_final['intent'][0][LAST_INPUT]
        intents= response_final['intent'][0][INTENTS]
        ...
        if (first_level_intent in intent_model_url):
            ...
            api_response1 = getClassificationAPIResponse( params, 'model_url',line_no)
            api_response = api_response1['eventData']['malResponse']
...
if __name__=="__main__":
    ...
    request_url = config.get('params','root_model_url')
    scxml_url = config.get('params','scxml_url')
    ...
    intent_model_url['requestUrl'] = request_url
    intent_model_url['scxml'] = scxml_url
    intent_model_url['web2nlKey'] = config.get('params','web2nlKey')
    # else:
