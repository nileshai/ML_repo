...
#tellme_username=jgeorge
tellme_username=$USER

#FAFR19=$USER@$host2
FAFR19=$tellme_username@$host2
#if [ $CFLAG -ne 0 ]; then
...
output=`ssh $tellme_username@$host $command`
if [ ! output ]; then
...
ssh $tellme_username@$host $command;

echo "Copying files to $host:$tmp_dir"
echo

## Copy files to tmp directory
scp -q $slm_file $ssi_file $ssi_params_file $tellme_username@$host:$tmp_dir 
...
scp -q $tellme_username@$host:$tmp_dir\\ssi.cfr $OUTDIR/ssi.cfr
scp -q $tellme_username@$host:$tmp_dir\\compiled_model.cfg $OUTDIR/compiled_model.cfg

command="scp -q $tellme_username@$host:$tmp_dir\\ssi.cfr $OUTDIR"

echo "Cleaning up $host for tmp files"
echo 

ssh $tellme_username@$host Remove-Item -Force -Recurse $tmp_dir
