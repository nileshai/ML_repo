...
#output=`bash $SRCDIR/convert_to_arpa.sh $INPDATA $INPARPA`
#output=`ssh $host1 bash $SRCDIR/convert_to_arpa.sh $INPDATA $INPARPA`

echo "adding backoff weights"
python $SRCDIR/add_backoff.py $INPARPA

command="(/usr/local/bin/NGramToCFGTool.exe -i $INTERARPA -t ARPA -o inter.cfg -b \"<s>\" -e \"</s>\" -l 1033 -r Top -sf W3C -inittag \"out={}; out.classsubstitution = gSubstitutions = [];\")"
output=`$command`
